...
def formatear_sku(sku):
    ...
    # Quitamos decimales si Excel lo leyó como número
    sku_str = str(sku).strip().split('.')[0]
    if sku_str.isdigit() and len(sku_str) < 5:
        return sku_str.zfill(5)
    return sku_str

# 2. Función de carga exclusiva para Excel (.xlsx)
...
st.title("📦 Gestión de Stock y Handling Times")
...
if st.button(f"🚀 GENERAR ACTUALIZACIÓN PARA {tienda.upper()} {pais}"):
    if not (f_listing and f_massalaves and f_hb and f_aux):
        ...
        try:
            ...
            # Decidir qué Handling Times usar
            ht_actual = mapas_ht_memoria[pais]
            if f_ht_subida:
                df_ht_extra = cargar_excel(f_ht_subida)
                if df_ht_extra is not None:
                    # Asume Col A: Plantilla, Col B: Valor HT
                    ht_actual = {str(r[0]).strip().lower(): int(r[1]) for r in df_ht_extra.values}
            ...
            st.success(f"✅ ¡Hecho! Usando Handling Times de {pais}.")
            st.dataframe(final.head(20))
            
            txt_output = final.to_csv(sep='\t', index=False)
            st.download_button(f"📥 Descargar STOCK_{tienda}_{pais}.txt", txt_output, f"STOCK_{tienda}_{pais}.txt")
